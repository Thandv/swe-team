...
- End your turn with `HANDOFF: qa — <what to test>` (typical), or `HANDOFF: architect — <design gap>`, or `HANDOFF: user — <question>` if blocked. The orchestrator records the log entry — you don't touch `BUILD_LOG.json`.
